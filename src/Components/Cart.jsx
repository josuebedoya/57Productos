  ...
  return (
   <div id='CartDropdown' onClick={ () => handleOpenModal() }>
     <div ref={ modalRef }
       ...
       {
         openModal ? (
          <div className={ `${ openModal ? 'animate-fade-in' : 'animate-fade-out' } cursor-auto cart-products absolute px-6 rounded-lg shadow-custom w-64 -left-40 top-10 bg-white min-h-20 max-h-[373px] ${ totalAmountProducts && ( 'overflow-y-scroll scrollbar-thin scrollbar-thumb-scrollbar scrollbar-track-transparent' ) }` }
           onClick={ ( e ) => e.stopPropagation() }>
            { totalAmountProducts > 0 ? (
             <>
               <div className='top-section flex justify-between items-end text-Primary text-15 border-b border-b-gray-200 pb-1 sticky top-0 bg-white pt-4'>
               ...
               <div className='list-section cart-list pt-4'>
                 {
                   items.map( ( item, index ) => (
                    <div key={ index }  className='item-product flex flex-wrap  mb-3 pb-3 border-b border-b-gray-200 last:mb-0 last:pb-2'>
                      ...
                      <div className='information w-4/6 group/info pl-2'>
                        <Link to={ 'tienda/' + Slug(item.title) } target='_self'>
                          <h5 className='title text-Primary text-13 family-oswald group-hover/info:text-Secondary group-hover/info:underline duration-150 cursor-pointer'>
                            { item.title }
                          </h5>
                        </Link>
               ...
               <div className='bottom-section flex justify-between sticky -bottom-0.5 py-4 bg-white tracking-'>
                 <Button classBtn='text-xs  pr-5 tracking-wider' iconRight icon={ <TicketMoney classIcons='text-lg'/> } onClick={()=>{goToPayments(); setOpenModal(false)}}>
                   Comprar Todo
                 </Button>
