...
const Get = async <T extends Tables>(q: GetParams<T>): Promise<ResGet<T>> => {
  const {table} = q;
  ...
  try {
    ...
    const resData = error
      ? null
      : {
        items: (data ?? []) as unknown as TablesObj[T][],
        count: count ?? 0,
        page: q.page ?? 1,
    ...
    return response as unknown as ResGet<T>;
  } catch (err) {
    return Response<T>({
      data: null,
      success: false,
