...
const getImages = require.context( '../Resources/Images', true, /\.(jpg|png)$/ ); // get img
const InfoAbout = () => {
  const imgs = [
    <img src={ getImages( './logo.png' ) } alt="img-1" className=' object-cover'/>,
    <img src={ getImages( './logo.png' ) } alt="img-1" className='object-cover'/>,
    <img src={ getImages( './logo.png' ) } alt="img-1" className='object-cover'/>,
    <img src={ getImages( './logo.png' ) } alt="img-1" className='object-cover'/>,
    <img src={ getImages( './logo.png' ) } alt="img-1" className='object-cover'/>,
     ...
     <div className='container mx-auto px-3'>
       <div className='flex flex-col tl:flex-row tl:justify-center tl:items-center'>
         ...
         <div className='carousel tl:flex-1 w-full tl:w-1/2 tl:pr-5 lg:pl-7 rounded-3xl max-w-full order-1 tl:order-2'>
           <Carousel items={ imgs } responsive={ responsive } itemsView={ 'auto' } itemsSpace={ 5 } effect={ 2 } loop dots cursor itemClass='w-52 h-52 md:h-64 md:w-64 2xl:w-64 2xl:h-64 rounded-lg shadow-lg overflow-hidden'/>
         </div>
       </div>
     </div>
   </section>
